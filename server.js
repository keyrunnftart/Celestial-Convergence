...
wss.on('listening', () => {
  console.log('\n╔══════════════════════════════════════════╗');
  console.log('║   Celestial Convergence Bridge v1         ║');
  console.log('╚══════════════════════════════════════════╝');
  console.log(`WS      : ws://localhost:${WS_PORT}`);
  console.log(`Mode    : ${MOCK ? `MOCK (${MOCK_AUCTION_SECS}s auction, bid every ${MOCK_BID_MS/1000}s)` : 'LIVE (Etherlink Shadownet)'}`);
  if(!MOCK){
...
function startMockAuction() {
  ...
  const tick = setInterval(()=>{
    ...
    if(mockSecondsLeft<=0){
      ...
      broadcast({ type:'AUCTION_END', winner:currentTopBidder, finalBidCount:bidCount, generation:3, chain:'Etherlink' });
    }
  }, 1000);
